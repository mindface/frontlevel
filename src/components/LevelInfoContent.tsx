            ...
            <Box sx={{maxWidth: '50%', width: '100%', mb: 3, p: 2}}>
              <div className="p-3">
                ...
                <Slider
                  ...
                  value={value01}
                  step={10}
                  onChange={(e:Event, value: number | number[]) => {if(typeof value === 'number') value01Set(value);}}
                  marks={marks}
                  valueLabelDisplay="on"
                />
              </div>
              {props.viewId > 1 && <div className="p-3">
                <p className="caption">ツール化した情報</p>
                <Slider
                  aria-label="Always visible"
                  value={value02}
                  onChange={(e:Event, value: number | number[]) => {if(typeof value === 'number') value02Set(value);}}
                  step={10}
                  marks={marks}
                  valueLabelDisplay="on"
                />
              </div>}
              {props.viewId > 2 && <div className="p-3">
                <p className="caption">状況分析</p>
                <Slider
                  aria-label="Always visible"
                  value={value03}
                  onChange={(e:Event, value: number | number[]) => { if(typeof value === 'number') value03Set(value);}}
                  step={10}
                  marks={marks}
                  valueLabelDisplay="on"
                />
