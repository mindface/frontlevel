...
function NumberContent() {
  ...
  const goalRate = () => {
    return levelinfo.target !== 0?Math.ceil(Number(levelinfo.target) / Number(levelinfo.verification)*1000)/10 : 0;
  };

  const infoProcess = () => {
    return levelinfo.info !== 0?Math.ceil((levelinfo.info) * (levelinfo.verification)*10)/10 : 0;
  }

  const userDiscRate= () => {
    const rate = Math.ceil((userLevel * levelinfo.memory)/levelinfo.verification*10)/1000;
    return rate;
  }
  ...
  return (
    <div className={switcher?"content content--number boxShadow p-2":"content content--number boxShadow p-2 off"} >
      <FormGroup>
        <FormControlLabel
          control={<Switch
                     defaultChecked
      ...
      <ul className="list _flex_ p-3">
        ...
        <li className="item mb-2">
          <Card sx={{ maxWidth: 345 }}>
            ...
            <p className="number">{metaRate()}(言語情報量量/ユーザーレベル)</p>
            </div>
          </Card>
        </li>
      </ul>
