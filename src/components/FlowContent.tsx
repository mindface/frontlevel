...
function FlowContent(props:SetProps) {
  ...
  return (
    <div className="view-flow-info p-4" style={{ minWidth: steps.length * 180 + 'px' }}>
      <Box sx={{ width: '100%' }}>
        ...
        <div className="patterm-info">
          <React.Fragment>
            <Typography sx={{ mt: 2, mb: 1 }}>情報レベルStep {activeStep + 1}</Typography>
            <Button onClick={handleClickOpen('paper')}>改善情報リスト Level {activeStep + 1}</Button>
            <Dialog
              fullScreen
              open={open}
              onClose={handleClose}
            ...
            >
              <DialogTitle id="scroll-dialog-title">変更項目{activeStep + 1}</DialogTitle>
              <DialogContent dividers={scroll === 'paper'}>
                  <Card
                    component="div"
                    sx={{ display: 'flex', flex: 1, alignItems: 'center', position: 'relative' ,m: 4, p: '2px 4px', border: 'none', overflow: "visible" }}
                  >
                    <InputBase
                      ...
                      onChange={(e:React.ChangeEvent<HTMLInputElement>) => { searchLinkTextSet(e.target.value) }}
                    />
                    <IconButton type="submit" sx={{ p: '10px' }} aria-label="search" onClick={setJsonData}>
                      <SearchIcon />
                    </IconButton>
                        ...
                        {searchLinkList.map((item:SearchText) => {
                          return (
                            <ListItem disablePadding>
                              <ListItemButton onClick={() => { window.open(`https://www.google.com/search?q=${item.text}`,'_blank') }} >
                ...
                <div className="_flex_">
                  {data[activeStep].list.map((item:string, index:number) => {
                    return (<div key={index} className="flow-box p-1">
                      <p className="text pb-1">{item}</p>
                      {<div className="img-box _flex_">
                        <img src={`pattern/${setPath(index+1)}.png`} className="img" alt="" />
                      </div>}
                    </div>)
                  })}
                </div>
